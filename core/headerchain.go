...
type HeaderChain struct {
	...
	tdCache     *lru.Cache // Cache for the most recent block total difficulties   //hash:td
	numberCache *lru.Cache // Cache for the most recent block numbers   //hash: number
...
func (hc *HeaderChain) GetBlockNumber(hash common.Hash) uint64 {
	if cached, ok := hc.numberCache.Get(hash); ok {  //hash: number
		return cached.(uint64)
	}
	number := GetBlockNumber(hc.chainDb, hash) //read from db
	if number != missingNumber {
		hc.numberCache.Add(hash, number) //add to cache
	}
	return number
}
...
func (hc *HeaderChain) GetTd(hash common.Hash, number uint64) *big.Int {
	...
	if cached, ok := hc.tdCache.Get(hash); ok { //cache    hash:td
		return cached.(*big.Int)
	}
	td := GetTd(hc.chainDb, hash, number) //get from database
	if td == nil {
		return nil
	}
	// Cache the found body for next time and return
	hc.tdCache.Add(hash, td) //add back to tdCache
	return td
}
...
func (hc *HeaderChain) GetHeaderByNumber(number uint64) *types.Header {
	fmt.Printf("%s\n", "test headerchain.go GetHeaderByNumber")
