...
//通过number得到block的hash
//在数据的key为  "h" + number + "n"
// GetCanonicalHash retrieves a hash assigned to a canonical block number.
//相应的方法:WriteCanonicalHash, DeleteCanonicalHash
func GetCanonicalHash(db DatabaseReader, number uint64) common.Hash {
	fmt.Printf("test database_util.go GetCanonicalHash number = number %s\n", number)
	fmt.Println(db)
	data, _ := db.Get(append(append(headerPrefix, encodeBlockNumber(number)...), numSuffix...)) //h + number + n
	if len(data) == 0 {
		return common.Hash{}
	}
...
func GetBlockNumber(db DatabaseReader, hash common.Hash) uint64 {
	data, _ := db.Get(append(blockHashPrefix, hash.Bytes()...))  //"H" + hash
	if len(data) != 8 {
...
func GetHeadHeaderHash(db DatabaseReader) common.Hash {
	data, _ := db.Get(headHeaderKey) //"LastHeader"
	if len(data) == 0 {
		return common.Hash{}
	}
	return common.BytesToHash(data)
...
func GetHeader(db DatabaseReader, hash common.Hash, number uint64) *types.Header {
	data := GetHeaderRLP(db, hash, number)  //从数据库读取
	if len(data) == 0 {
		return nil
	}
...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	header := GetHeader(db, hash, number) //从数据库读取header
	if header == nil {
		return nil
	}
	body := GetBody(db, hash, number) //从数据库读取body
	if body == nil {
		return nil
	}
...
func WriteHeader(db ethdb.Putter, header *types.Header) error {
	data, err := rlp.EncodeToBytes(header) //header -> RLP
	if err != nil {
		return err
	}
...
func WriteBody(db ethdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
	data, err := rlp.EncodeToBytes(body) //body ->RLP
	if err != nil {
		return err
	}
...
func WriteTd(db ethdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
	data, err := rlp.EncodeToBytes(td) //td -> RLP
	if err != nil {
		return err
	}
	//key: 'h' + num + hash + 't'
