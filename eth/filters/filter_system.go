...
// broadcast event to filters that match criteria(标准).
func (es *EventSystem) broadcast(filters filterIndex, ev interface{}) {
	...
	case core.TxPreEvent:
		for _, f := range filters[PendingTransactionsSubscription] {
			f.hashes <- e.Tx.Hash() //filters/api.go  notifier.Notify(rpcSub.ID, h)
		}
...
func (es *EventSystem) eventLoop() {
	...
	for {
		...
		case ev := <-txCh:
			fmt.Printf("%s\n", "test filter_system.go eventLoop handle TxPreEvent")
			es.broadcast(index, ev)
		case ev := <-rmLogsCh:
			es.broadcast(index, ev)
		case ev := <-logsCh:
